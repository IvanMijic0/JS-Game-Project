window.addEventListener('load', function (){
    ...
    class Particle {
        ...
        draw(context){
            ...
            context.drawImage(this.image, this.frameX * this.spriteSize, this.frameY * this.spriteSize,
                this.spriteSize, this.spriteSize, this.size * -.5, this.size * -.5, this.size, this.size)
            context.restore();
        }
    }
    class Player{
        ...
        VerticalMovement() {
            if (this.game.keys.includes("w") || this.game.keys.includes("ArrowUp")) this.speedY = -this.maxSpeed;
            else if (this.game.keys.includes("s") || this.game.keys.includes("ArrowDown")) this.speedY = this.maxSpeed
            else this.speedY = 0
            this.y += this.speedY;
            // vertical boundaries
    ...
    class UI{
        ...
        draw(context){
            ...
            context.font = this.fontSize.toString().concat("px ").concat(this.fontFamily.toString());
            // score
            context.fillText("Score: ".concat(this.game.score.toString()), 20, 40);
            // timer
            const formattedTime = (this.game.gameTime * .001).toFixed(1)
            context.fillText("Timer: ".concat(formattedTime.toString()), 20, 100);
            // game over messages
            if (this.game.gameOver){
                ...
                context.font = "80px ".concat(this.fontFamily.toString());
                context.fillText(message1, this.game.width * .5, this.game.height * .5 - 20);

                context.font = "35px ".concat(this.fontFamily.toString());
    ...
    class Game{
        ...
        update(deltaTime){
            ...
            this.rechargeProjectiles(deltaTime);
        ...
        addExplosion(enemy){
            const randomize = Math.random();
            if (randomize < .5) this.explosions.push(new SmokeExplosion(this,
                enemy.x + enemy.width * .5, enemy.y + enemy.height * .5))
            else this.explosions.push(new FireExplosion(this,
                enemy.x + enemy.width * .5, enemy.y + enemy.height * .5))
        }
        ...
        checkEnemyCollision(enemy) {
            if (this.checkCollision(this.player, enemy)) {
                ...
                for (let i = 0; i < enemy.score; i++) {
                    this.particles.push(new Particle(this, enemy.x + enemy.width * .5,
    ...
    animate(0); // auto-generates a timeStamp value
})
